...
const timelineTreeProvider_1 = require("./timelineTreeProvider");
const analyzerTreeProvider_1 = require("./analyzerTreeProvider");
const scoutConsole_1 = require("./scoutConsole");
const scoutAnalyzerPanel_1 = require("./scoutAnalyzerPanel");
const consoleLinksProvider_1 = require("./consoleLinksProvider");
...
function activate(context) {
    ...
    scoutConsole = new scoutConsole_1.ScoutConsole();
    ...
    scoutConsole.logMessage("╚════════════════════════════════════════╝");
    scoutConsole.logMessage("");
    scoutConsole.logMessage("💡 Tip: Click on file:line links to navigate to source");
    scoutConsole.logMessage("");
    // Register console navigation command
    (0, consoleLinksProvider_1.registerConsoleNavigationCommand)(context);
    ...
    patternEngine = new patternEngine_1.PatternEngine();
    // Initialize diagnostics provider
    const diagnosticCollection = vscode.languages.createDiagnosticCollection("log-scout-analyzer");
    diagnosticsProvider = new diagnosticsProvider_1.DiagnosticsProvider(diagnosticCollection, patternEngine, outputChannel, updateStatusBar);
    context.subscriptions.push(diagnosticCollection);
    // Pass diagnostics provider to ScoutAnalyzerPanel
    scoutAnalyzerPanel_1.ScoutAnalyzerPanel.setDiagnosticsProvider(diagnosticsProvider);
    ...
    timelineTreeProvider = new timelineTreeProvider_1.TimelineTreeProvider();
    ...
    analyzerTreeProvider_1.AnalyzerTreeProvider.setDiagnosticsProvider(diagnosticsProvider);
    ...
    updateStatusBar();
    // Register commands
    const analyzeCommand = vscode.commands.registerCommand("logScoutAnalyzer.analyzeFile", () => {
        ...
        if (editor &&
            ...
            // Log to console
            scoutConsole.logAnalysisStart(editor.document.fileName, editor.document.uri);
            scoutConsole.show();
            outputChannel.appendLine("");
            outputChannel.appendLine(`[${"=".repeat(50)}]`);
            outputChannel.appendLine(`[${timestamp}] 🔍 MANUAL ANALYSIS TRIGGERED`);
            ...
            const results = diagnostics.map((diag) => {
                ...
                if (scoutConsole) {
                    scoutConsole.logResult(severity, diag.range.start.line, mainMessage, category, extractedTimestamp, editor.document.uri);
                }
                return {
                    severity,
            ...
            // Log completion
            scoutConsole.logAnalysisComplete(errorCount, warningCount, infoCount, debugCount, duration);
            // Log summary
            const categories = [
            ...
            outputChannel.appendLine(`✓ Analysis complete: Found ${diagnosticCount} issue(s)`);
            outputChannel.appendLine("  → Check Scout Analyzer sidebar for tree views");
            outputChannel.appendLine("  → Check Scout Console for real-time output");
            outputChannel.appendLine("  → Check Problems panel (Ctrl+Shift+M) for details");
            outputChannel.appendLine("");
            // Still show webview panel if enabled
            const config = vscode.workspace.getConfiguration("logScoutAnalyzer");
            const showPanel = config.get("showResultsPanel", true);
            if (showPanel) {
                const panel = resultsPanel_1.ResultsPanel.createOrShow(context.extensionUri, editor.document);
                panel.updateResults(results.map((r) => ({
                    severity: r.severity,
                    line: r.line,
                    column: r.column,
                    message: r.message,
                    ...
                    timestamp: r.timestamp,
                    category: r.category,
                })), editor.document.fileName, new Date().toLocaleString());
            }
            vscode.window.showInformationMessage(`Analysis complete! 🔴 ${errorCount} 🟡 ${warningCount} 🔵 ${infoCount} | Check Scout Analyzer in left sidebar`);
            // Update status bar
            updateStatusBar();
        }
        else {
    ...
    const showPatternsCommand = vscode.commands.registerCommand("logScoutAnalyzer.showPatterns", () => {
        if (patternEngine && outputChannel) {
            const patterns = patternEngine.getAllPatterns();
            outputChannel.appendLine(`[${new Date().toISOString()}] Showing patterns configuration`);
            outputChannel.appendLine(`  Errors: ${patterns.errors.length} patterns`);
            outputChannel.appendLine(`  Warnings: ${patterns.warnings.length} patterns`);
            outputChannel.appendLine(`  Info: ${patterns.info.length} patterns`);
            const panel = vscode.window.createWebviewPanel("logScoutPatterns", "Log Scout Patterns", vscode.ViewColumn.One, {});
            panel.webview.html = generatePatternsHtml(patterns);
        }
    });
    const showVersionCommand = vscode.commands.registerCommand("logScoutAnalyzer.showVersion", () => {
        if (outputChannel) {
            outputChannel.show();
            outputChannel.appendLine("");
            outputChannel.appendLine("╔════════════════════════════════════════╗");
            ...
            outputChannel.appendLine(`🏷️  Version: ${buildInfo_1.BUILD_INFO.version}`);
            ...
            outputChannel.appendLine(`🎯 Pattern Count: ${patternEngine?.getPatternCount() || 0}`);
            outputChannel.appendLine("");
        }
        vscode.window.showInformationMessage(`${(0, buildInfo_1.getBuildInfo)()}\nBuild: ${buildInfo_1.BUILD_INFO.buildTimestamp}\nBuild #: ${buildInfo_1.BUILD_INFO.buildNumber}`);
    });
    ...
    const showConsoleCommand = vscode.commands.registerCommand("logScoutAnalyzer.showConsole", () => {
        if (scoutConsole) {
            scoutConsole.show();
            vscode.window.showInformationMessage("Scout Console opened in bottom panel. File paths are clickable (Ctrl+Click).");
        }
    });
    ...
    const analyzeDirectoryCommand = vscode.commands.registerCommand("logScoutAnalyzer.analyzeDirectory", async () => {
        ...
        scoutConsole.logMessage(`✓ Analyzed ${analyzed} files in directory`);
        vscode.window.showInformationMessage(`Analyzed ${analyzed} files. Check Scout Analyzer in left sidebar for results.`);
    });
    ...
    const toggleSignificantEventsCommand = vscode.commands.registerCommand("logScoutAnalyzer.toggleSignificantEvents", () => {
        const config = vscode.workspace.getConfiguration("logScoutAnalyzer");
        const currentValue = config.get("timeline.showSignificantEvents", true);
        config
            .update("timeline.showSignificantEvents", !currentValue, vscode.ConfigurationTarget.Global)
            .then(() => {
            timelineTreeProvider?.setShowSignificantEvents(!currentValue);
            vscode.window.showInformationMessage(`Timeline now showing: ${!currentValue ? "Significant Events" : "All Issues"}`);
        });
    });
    // Show Timeline Visualization command
    const showTimelineVisualizationCommand = vscode.commands.registerCommand("logScoutAnalyzer.showTimelineVisualization", () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage("No active editor");
            return;
        ...
        if (timelineEvents.length === 0) {
            vscode.window.showInformationMessage("No timeline events with timestamps found");
            return;
        }
        timelineVisualization?.showTimelinePanel(timelineEvents, editor.document);
        vscode.window.showInformationMessage(`Timeline visualization opened with ${timelineEvents.length} events`);
    });
    // Show SIP Ladder Diagram command
    const showLadderDiagramCommand = vscode.commands.registerCommand("logScoutAnalyzer.showLadderDiagram", () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage("No active editor");
            return;
        ...
        const sipEvents = sipCallFlowParser_1.SipCallFlowParser.parseSipMessages(editor.document);
        if (sipEvents.length === 0) {
            vscode.window.showInformationMessage("No SIP/call flow messages detected. Make sure your logs contain SIP protocol messages or call events.");
            return;
        }
        timelineVisualization?.setLadderEvents(sipEvents);
        timelineVisualization?.showLadderDiagram();
        vscode.window.showInformationMessage(`SIP Ladder Diagram opened with ${sipEvents.length} messages`);
    });
    // Register all commands
    context.subscriptions.push(analyzeCommand, clearCommand, showPatternsCommand, showVersionCommand, jumpToLineCommand, refreshResultsCommand, showConsoleCommand, clearConsoleCommand, groupBySeverityCommand, groupByCategoryCommand, groupByFileCommand, resetViewCommand, exportResultsCommand, analyzeDirectoryCommand, analyzeAllBelowCommand, openAnalyzerPanelCommand, openScoutViewCommand, toggleConsoleLocationCommand, openSplitViewCommand, closeSplitViewCommand, setTimeframeCommand, toggleSignificantEventsCommand, showTimelineVisualizationCommand, showLadderDiagramCommand);
    // Register document change listeners
    const onDidOpenTextDocument = vscode.workspace.onDidOpenTextDocument((document) => {
        if (isLogFile(document) && diagnosticsProvider && outputChannel) {
            outputChannel.appendLine("");
            outputChannel.appendLine(`[${new Date().toISOString()}] 📂 Auto-analyzing opened file`);
            outputChannel.appendLine(`   → ${document.fileName}`);
            // Show prompt for analysis
            const config = vscode.workspace.getConfiguration("logScoutAnalyzer");
            const autoPrompt = config.get("autoPromptOnOpen", true);
            if (autoPrompt) {
                vscode.window
                    .showInformationMessage(`Log file detected: ${document.fileName.split(/[\\/]/).pop()}`, "Analyze Now", "Analyze Later", "Don't Ask Again")
                    .then((choice) => {
                    if (choice === "Analyze Now" &&
                        ...
                        config.update("autoPromptOnOpen", false, vscode.ConfigurationTarget.Global);
                    }
                });
            }
            else {
    ...
    context.subscriptions.push(onDidOpenTextDocument, onDidChangeTextDocument, onDidSaveTextDocument, onDidChangeActiveTextEditor);
    ...
    if (outputChannel && analyzedCount > 0) {
        outputChannel.appendLine(`✓ Auto-analyzed ${analyzedCount} open log file(s)`);
        outputChannel.appendLine("");
    }
    if (outputChannel) {
        ...
        outputChannel.appendLine("   📺 Scout Console available in bottom panel");
        outputChannel.appendLine("─".repeat(60));
        outputChannel.appendLine("");
    }
...
function deactivate() {
    ...
    console.log(`${(0, buildInfo_1.getBuildInfo)()} deactivated`);
