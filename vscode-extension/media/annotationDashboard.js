...
let minimapMode = 'filter'; // 'filter' shows only filtered, 'range' shows all with highlight
let searchQuery = '';
let searchIsRegex = false;
let logMinTime = 0; // Earliest log timestamp in milliseconds
let logMaxTime = 0; // Latest log timestamp in milliseconds
let useServerSideFiltering = false; // Dynamic: true for large datasets (5000+)
let totalAnnotationCount = 0; // Total count from server
let lastLoggedFilterStrategy = null; // Track previous strategy to avoid spam
 ...
 */
function renderTimelineGraphic(timestamp) {
    try {
...
function applyFilterState(state) {
    ...
    if (filterError)
        filterError.checked = state.filterError;
    if (filterWarning)
        filterWarning.checked = state.filterWarning;
    if (filterInfo)
        filterInfo.checked = state.filterInfo;
    // Apply sort dropdown
...
function updateHiddenCategoriesUI(hiddenCategories) {
...
function initializeEventListeners() {
    ...
    // Minimap mode toggle
    const toggleMinimapMode = document.getElementById('toggle-minimap-mode');
...
function handleCardMenuToggle(event) {
...
function handleHideCategory(event) {
    const button = event.currentTarget;
...
function handleSeverityToggle(event) {
    const button = event.currentTarget;
...
function handleFilterChange() {
    ...
    cards.forEach(card => {
        ...
        const logLevelMatch = logLevelFilters[logLevel] ?? true;
        if (!logLevelMatch) {
            cardElement.classList.add('filtered-out');
            return;
        }
        // Check priority filter
        const priorityMatch = priorityFilters[priority] ?? true;
        if (!priorityMatch) {
            cardElement.classList.add('filtered-out');
            return;
        }
    ...
    cards.sort((a, b) => {
        ...
        switch (sortBy) {
            case 'priority': {
                ...
                const prioA = priorityOrder[(cardA.getAttribute('data-priority') || '').toLowerCase()] || 999;
                const prioB = priorityOrder[(cardB.getAttribute('data-priority') || '').toLowerCase()] || 999;
                return prioA - prioB;
            }
            case 'category': {
...
function handleCopyMatchedText(event) {
    const button = event.currentTarget;
 ...
 */
function setTimeRangeFromAnnotations(annotations) {
    if (annotations.length === 0)
        return;
 ...
 */
function renderWithVirtualScroll(annotations, container) {
    ...
    function updateVisibleCards() {
        ...
        const endIndex = Math.min(annotations.length, Math.ceil((scrollTop + viewportHeight) / cardHeight) + bufferSize);
 ...
 */
function renderMinimap(filteredAnnotations) {
...
//# sourceMappingURL=annotationDashboard.js.map