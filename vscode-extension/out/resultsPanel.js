...
class ResultsPanel {
    ...
    static createOrShow(extensionUri, document) {
        ...
        const panel = vscode.window.createWebviewPanel("logScoutResults", "Scout Analysis Results", column, {
            enableScripts: true,
    ...
    updateResults(results, fileName, timestamp) {
        this.allResults = results;
        this.fileName = fileName;
        this.analysisTime = timestamp;
        this.panel.webview.html = this.getHtmlContent(results, fileName, timestamp);
    }
    applyFilters(filters) {
        ...
        if (filters.keyword && filters.keyword.trim() !== "") {
            ...
            filtered = filtered.filter((r) => r.message.toLowerCase().includes(keyword) ||
        ...
        this.panel.webview.html = this.getHtmlContent(filtered, this.fileName, this.analysisTime);
    }
    ...
    getHtmlContent(results, fileName, timestamp) {
        ...
        const timestamps = results
            .map((r) => r.timestamp)
            .filter((t) => t);
        const minDate = timestamps.length > 0
            ? new Date(Math.min(...timestamps.map((t) => t.getTime())))
            : null;
        const maxDate = timestamps.length > 0
