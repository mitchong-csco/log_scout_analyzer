...
class TimelineVisualizationProvider {
            ...
            this.panel.webview.onDidReceiveMessage((message) => {
                switch (message.command) {
                    ...
                    case "showLadderDiagram":
                        this.showLadderDiagram(message.startLine, message.endLine);
                        break;
                }
            }, null, this.disposables);
        }
     ...
     */
    showLadderDiagram(startLine, endLine) {
        const events = startLine && endLine
            ? this.ladderEvents.filter((e) => e.line >= startLine && e.line <= endLine)
            : this.ladderEvents;
        if (events.length === 0) {
            vscode.window.showInformationMessage("No SIP/call flow events detected in the selected range");
            return;
        }
        if (this.panel) {
            this.panel.reveal(vscode.ViewColumn.Beside);
    ...
    getTimelineHtml(events, document) {
        ...
        const eventItems = sortedEvents
            ...
            if (index > 0) {
                ...
                const diffMs = event.timestamp.getTime() -
...
<body>
     ...
     */
    getSipMessageColor(method, responseCode) {
        if (responseCode) {
            if (responseCode >= 200 && responseCode < 300)
                return "#10b981"; // Success - green
            if (responseCode >= 300 && responseCode < 400)
                return "#3b82f6"; // Redirect - blue
            if (responseCode >= 400 && responseCode < 500)
                return "#f59e0b"; // Client error - orange
            if (responseCode >= 500)
...
exports.TimelineVisualizationProvider = TimelineVisualizationProvider;
//# sourceMappingURL=timelineVisualization.js.map