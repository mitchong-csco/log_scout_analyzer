...
class ScoutAnalyzerPanel {
    ...
    static createOrShow(extensionUri, scenarioManager, patternOverrideManager) {
        ...
        const panel = vscode.window.createWebviewPanel("scoutAnalyzer", "Scout Analyzer", column, {
            enableScripts: true,
            retainContextWhenHidden: true,
            localResourceRoots: [extensionUri],
        });
        ScoutAnalyzerPanel.currentPanel = new ScoutAnalyzerPanel(panel, scenarioManager, patternOverrideManager);
    }
    constructor(panel, scenarioManager, patternOverrideManager) {
        this._disposables = [];
        this._panel = panel;
    ...
    async _findLogFiles(dir, recursive) {
        const logFiles = [];
    ...
    async _createScenario(data) {
        try {
            ...
            const scenario = this.scenarioManager.createScenario(data.name, data.description, editor.document.uri.fsPath, data.lines, data.patterns);
            this._postMessage({
                command: "scenarioCreated",
                scenario,
            });
            vscode.window.showInformationMessage(`Scenario "${scenario.name}" created successfully`);
    ...
    async _deleteScenario(scenarioId) {
        try {
            if (!this.scenarioManager) {
                return;
            }
            ...
            const confirm = await vscode.window.showWarningMessage(`Delete scenario "${scenario.name}"?`, { modal: true }, "Delete");
            if (confirm === "Delete") {
    ...
    async _exportScenario(scenarioId) {
        try {
            if (!this.scenarioManager) {
                return;
            }
    ...
    _showCreationNotImplemented(type) {
        vscode.window.showInformationMessage(`${type} creation is coming soon! This will allow you to create custom ${type.toLowerCase()}s for your log analysis.`, "OK");
    }
    // Pattern management methods
    async _handleCreatePattern(data) {
        try {
            ...
            vscode.window.showInformationMessage(`Pattern "${pattern.name}" created successfully`);
            this._sendPatterns();
        }
        catch (error) {
    ...
    async _handleUpdatePattern(id, updates) {
        try {
            if (!this.patternOverrideManager) {
                return;
            }
    ...
    async _handleDeletePattern(id) {
        try {
            ...
            const confirm = await vscode.window.showWarningMessage(`Delete pattern "${pattern.name}"?`, { modal: true }, "Delete");
            if (confirm === "Delete") {
    ...
    async _handleResetPattern(sourceId) {
        try {
            if (!this.patternOverrideManager) {
                return;
            }
            const confirm = await vscode.window.showWarningMessage(`Reset pattern to MongoDB original?`, { modal: true }, "Reset");
            if (confirm === "Reset") {
    ...
    async _handleTogglePattern(id, enabled) {
        try {
            if (!this.patternOverrideManager) {
                return;
            }
...
exports.ScoutAnalyzerPanel = ScoutAnalyzerPanel;
//# sourceMappingURL=scoutAnalyzerPanel.js.map