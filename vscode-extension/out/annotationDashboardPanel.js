...
exports.AnnotationDashboardPanel = void 0;
...
class AnnotationDashboardPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('annotationDashboard', 'Annotation Dashboard', column, {
            enableScripts: true,
            localResourceRoots: [
    ...
    async _loadFilteredAnnotations(filters) {
    ...
    async _fetchAnnotationsFromDiagnostics() {
        const annotations = [];
        ...
        for (const [uri, diagnostics] of allDiagnostics) {
            ...
            try {
                ...
                for (const diagnostic of diagnostics) {
                    ...
                    const diagnosticAny = diagnostic;
    ...
    _extractPatternName(diagnostic) {
        if (diagnostic.code) {
            return String(diagnostic.code);
        }
        return 'Pattern Match';
    }
    _mapPriority(severity) {
        switch (severity) {
    ...
    _extractSeverityTrigger(diagnostic) {
    ...
    _interpolateFields(message, fields) {
    ...
    _getHtmlForWebview(webview) {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'annotationDashboard.js'));
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'annotationDashboard.css'));
...
<body>
    ...
    _hideCategory(category) {
        ...
        const hiddenCategories = config.get('dashboard.hiddenCategories', []);
    ...
    _sendFilterState() {
        ...
        this._panel.webview.postMessage({
            ...
            state: {
                filterError: config.get('dashboard.filterError', true),
                filterWarn: config.get('dashboard.filterWarn', true),
                filterInfo: config.get('dashboard.filterInfo', true),
                filterDebug: config.get('dashboard.filterDebug', true),
                filterTrace: config.get('dashboard.filterTrace', true),
                filterCritical: config.get('dashboard.filterCritical', true),
                filterHigh: config.get('dashboard.filterHigh', true),
                filterNormal: config.get('dashboard.filterNormal', true),
                filterLow: config.get('dashboard.filterLow', true),
                sortBy: config.get('dashboard.sortBy', 'time'),
                hiddenCategories: config.get('dashboard.hiddenCategories', [])
            }
        });
    }
    _saveFilterState(state) {
        const config = vscode.workspace.getConfiguration('logScoutAnalyzer');
    ...
    async _exportAnnotationsToFile(data) {
        try {
...
exports.AnnotationDashboardPanel = AnnotationDashboardPanel;
function getNonce() {
    let text = '';
...
//# sourceMappingURL=annotationDashboardPanel.js.map