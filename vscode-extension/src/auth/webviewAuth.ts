...
export class WebViewAuthProvider {
  ...
  private authResolve: ((result: AuthResult | null) => void) | null = null;
  ...
  async authenticate(): Promise<AuthResult | null> {
    return new Promise((resolve) => {
      ...
      vscode.window.showInformationMessage(
        'Cisco Scripts authentication panel opened'
      );
    });
  }
    ...
    switch (message.command) {
      ...
      case 'openBrowser':
        await vscode.env.openExternal(
          vscode.Uri.parse('https://scripts.cisco.com/login')
        );
        this.panel?.webview.postMessage({
          command: 'browserOpened',
          message:
            'Login page opened in your browser. Please complete authentication and paste the authorization code above.',
        });
        break;
   ...
   */
  private async handleCodeExchange(code: string): Promise<void> {
    try {
      this.panel?.webview.postMessage({
        command: 'loading',
        show: true,
   ...
   * Extract cookies from response headers
   */
  private extractCookies(headers: any): Record<string, string> {
