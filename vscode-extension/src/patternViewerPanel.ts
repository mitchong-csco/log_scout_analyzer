...
export class PatternViewerPanel {
  public static currentPanel: PatternViewerPanel | undefined;
  ...
  public static createOrShow(
    ...
    PatternViewerPanel.currentPanel = new PatternViewerPanel(
      panel,
      extensionUri
    );
    PatternViewerPanel.currentPanel.updatePatterns(patterns);
  ...
  public updatePatterns(patterns: any) {
    this._panel.webview.html = this._getHtmlForWebview(patterns);
  }

  private _exportToJson(data: any) {
    ...
    vscode.window.showSaveDialog(options).then((fileUri) => {
      if (fileUri) {
        ...
        vscode.window.showInformationMessage(
          `Patterns exported to ${path.basename(fileUri.fsPath)}`
        );
      }
    });
  }
    ...
    <div class="filters">
        <select id="severityFilter" onchange="filterPatterns()">
            ...
            ${severities
              .map(
                (s) =>
                  `<option value="${s}">${s} (${bySeverity[s].length})</option>`
              )
              .join("")}
        </select>
        <select id="categoryFilter" onchange="filterPatterns()">
            ...
            ${categories
              .map(
                (c) =>
                  `<option value="${c}">${c} (${byCategory[c].length})</option>`
              )
              .join("")}
        </select>
        <select id="serviceFilter" onchange="filterPatterns()">
            ...
            ${services
              .map(
                (s) =>
                  `<option value="${s}">${s} (${byService[s].length})</option>`
              )
              .join("")}
        </select>
            ...
            <div class="pattern-card ${p.severity}" 
                  ...
                  p.captureFields && p.captureFields.length > 0 
                    ? `<div class="pattern-parameters">
                        ...
                        <div class="parameter-list">
                          ${p.captureFields.map((field: string) => `<span class="parameter">${field}</span>`).join('')}
                        </div>
                      </div>`
                    : ''
                }
                ${
                  p.parameterExtractors && p.parameterExtractors.length > 0
                    ? `<div class="pattern-parameters">
                        ...
                        <div class="parameter-list">
                          ${p.parameterExtractors.map((pe: any) => `<span class="parameter" title="${pe.regex}">${pe.name}</span>`).join('')}
                        </div>
                      </div>`
                    : ''
                }
                <div class="pattern-footer">
                    <div class="tags">
                        ...
                        ${(p.tags || []).map((tag: string) => `<span class="tag">${tag}</span>`).join("")}
                    </div>
                </div>
