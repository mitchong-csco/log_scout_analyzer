...
export class TimelineTreeProvider implements vscode.TreeDataProvider<TimelineTreeItem> {
    ...
    private getTimeBuckets(): TimelineTreeItem[] {
    ...
    private getSignificantEventBuckets(): TimelineTreeItem[] {
        if (this.significantEvents.length === 0) {
            return [
                new TimelineTreeItem(
                    "No timestamps detected",
                    "Timestamps not found in log lines",
        ...
        // Create time buckets
        const bucketMap = new Map<string, SignificantEvent[]>();
        ...
        const treeItems: TimelineTreeItem[] = [];
        ...
        for (const [bucketKey, events] of bucketMap.entries()) {
            ...
            if (errors > 0) tooltip += `🔴 ${errors} errors\n`;
            if (warnings > 0) tooltip += `🟡 ${warnings} warnings\n`;
            if (infos > 0) tooltip += `🔵 ${infos} info\n`;
            if (lifecycleEvents > 0)
                tooltip += `🔄 ${lifecycleEvents} lifecycle\n`;
            if (authEvents > 0) tooltip += `🔐 ${authEvents} auth\n`;
            if (callEvents > 0) tooltip += `📞 ${callEvents} calls\n`;
    ...
    private getResultBuckets(): TimelineTreeItem[] {
        ...
        // Create time buckets
        const bucketMap = new Map<string, ResultItem[]>();
        const intervalMs = this.timeInterval * 60 * 1000;
    ...
    private getTimeResults(bucket: TimelineTreeItem): TimelineTreeItem[] {
    ...
    private createSignificantEventItems(
    ...
    private createResultItems(results: ResultItem[]): TimelineTreeItem[] {
